...
FLASH_CR  = 0x4004A000
FLASH_GO  = 0x4004A014   #命令执行寄存器
FLASH_AR  = 0x4004A018   #地址寄存器
FLASH_CMD = 0x4004A024

FLASH_CR_CMDTYPE_Pos = 0
FLASH_CR_CMDTYPE_Msk = (0x0F << FLASH_CR_CMDTYPE_Pos)
FLASH_CR_CMDWREN_Pos = 5
FLASH_CR_MEMWREN_Pos = 12

FLASH_CMD_ERASE_SECT = 0x20
...
class SWM190(object):
    ...
    def __init__(self, xlink):
        super(SWM190, self).__init__()

        self.xlink = xlink

        self.xlink.reset()
        self.flash = Flash(self.xlink, SWM190_flash_algo)
    ...
    def sect_erase(self, addr, size):
        for i in range(0, size // self.SECT_SIZE):
            ...
            reg = self.xlink.read_U32(FLASH_CR)
            reg &= ~FLASH_CR_CMDTYPE_Msk
            reg |= (1 << FLASH_CR_CMDWREN_Pos) | (1 << FLASH_CR_MEMWREN_Pos) | (7 << FLASH_CR_CMDTYPE_Pos)
            self.xlink.write_U32(FLASH_CR, reg)

            self.xlink.write_U32(FLASH_CMD, FLASH_CMD_ERASE_SECT)

            self.xlink.write_U32(FLASH_GO, 1)
            while(self.xlink.read_U32(FLASH_GO)): pass
    ...
    def chip_read(self, addr, size, buff):
        data = self.xlink.read_mem_U32(0x41000000 + addr, size//4)

        for x in data:
            buff.extend([x & 0xFF, (x >> 8) & 0xFF, (x >> 16) & 0xFF, x >> 24])


class SWM190xB(SWM190):
    CHIP_SIZE = 0x1E000     # 120K


class SWM190xC(SWM190):
...
SWM190_flash_algo = {
    ...
    'pc_Init'            : 0x20000049,
    'pc_UnInit'          : 0x2000009D,
    'pc_EraseSector'     : 0x200000E1,
    'pc_ProgramPage'     : 0x200000EB,
    'pc_Verify'          : 0x20000111,
    'pc_EraseChip'       : 0x12000001F,
    'pc_BlankCheck'      : 0x20000145,
    'pc_Read'            : 0x12000001F,
